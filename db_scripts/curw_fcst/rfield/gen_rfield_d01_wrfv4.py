...
def gen_rfield_d01(model, version):
    ...
    try:
        ...
        while timestamp <= end_time :
            ...
            with connection.cursor() as cursor2:
                cursor2.callproc('get_d01_rfield', (model, version, timestamp))
                results = cursor2.fetchall()
                for result in results:
                    rfield.append('{} {} {}'.format(result.get('longitude'), result.get('latitude'), result.get('value')))

            if timestamp < now:
                write_to_file('/var/www/html/wrf/{}/rfield/d01/past/{}_{}_{}_rfield.txt'.format(version, model, version, timestamp), rfield)
            else:
                write_to_file('/var/www/html/wrf/{}/rfield/d01/future/{}_{}_{}_rfield.txt'.format(version, model, version, timestamp), rfield)
...
gen_rfield_d01("WRF_A", "v4")
gen_rfield_d01("WRF_C", "v4")
gen_rfield_d01("WRF_E", "v4")
gen_rfield_d01("WRF_SE", "v4")
