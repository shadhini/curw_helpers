CREATE DEFINER=`routine_user`@`%` PROCEDURE `getNearestFcstDISforGivenStation`(
IN model VARCHAR(25),
...
BEGIN
	...
	SET @tmsID = (SELECT curw_fcst.run.id FROM curw_fcst.run WHERE curw_fcst.run.source=@sourceID  AND curw_fcst.run.station=station_id 
		AND curw_fcst.run.sim_tag=sim_tag AND curw_fcst.run.variable=3 AND curw_fcst.run.unit=3);
	
	SET @fgt = (SELECT sorted.fgt FROM (SELECT
	temp.fgt,
    abs(expected_fgt - temp.fgt) as gap
	FROM 
    (select distinct curw_fcst.data.fgt as fgt from curw_fcst.data where curw_fcst.data.id=@tmsID) as temp
