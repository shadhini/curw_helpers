CREATE DEFINER=`routine_user`@`%` PROCEDURE `get_150_fcsts_for_given_grid`(
IN grid VARCHAR(25),
...
BEGIN
	SET @stationID = (SELECT curw_fcst.station.id FROM curw_fcst.station where id like '13_____'
	...
	SET @tmsID = (SELECT curw_fcst.run.id FROM curw_fcst.run
					WHERE curw_fcst.run.sim_tag = sim_tag AND curw_fcst.run.source = 10 
